...
const TaskManagement = () => {
    ...
    const handlePageChange = (setter, page, maxPage) => {
    ...
    const paginatedPendingTasks = pendingTasks.slice((pendingPage - 1) * tasksPerPage, pendingPage * tasksPerPage);
    const paginatedInProgressTasks = inProgressTasks.slice((inProgressPage - 1) * tasksPerPage, inProgressPage * tasksPerPage);
    const paginatedCompletedTasks = completedTasks.slice((completedPage - 1) * tasksPerPage, completedPage * tasksPerPage);

    // Total pages for each task list
    const totalPendingPages = Math.ceil(pendingTasks.length / tasksPerPage);
    const totalInProgressPages = Math.ceil(inProgressTasks.length / tasksPerPage);
    const totalCompletedPages = Math.ceil(completedTasks.length / tasksPerPage);
    ...
    return (
        <div className="relative flex h-screen">
            ...
            <div className="flex-1 ml-[15%] p-[2vw]">
                ...
                <div className="flex gap-[1.25vw]">
                    <div className="flex-1 flex flex-col gap-8">
                        {paginatedPendingTasks.map((item) => (
                            <TaskCard
                                key={item.task.id}
                                title={item.task.name}
                                assignee={item.employeeName}
                        ...
                        <div className="flex justify-between items-center mt-4">
                            <button
                                ...
                                onClick={() => handlePageChange(setPendingPage, pendingPage - 1, totalPendingPages)}
                            >
                                <img src='/arrow.png' width={10} height={10} />
                            </button>
                            <span>{pendingPage} of {totalPendingPages}</span>
                            <button
                                className="px-4 py-2 bg-gray-200 rounded"
                                disabled={pendingPage === totalPendingPages}
                                onClick={() => handlePageChange(setPendingPage, pendingPage + 1, totalPendingPages)}
                            >
                               <img src='/rightarrow.png' width={10} height={10} />
                            </button>
                        </div>
                    
                    </div>
                    <div className="flex-1 flex flex-col gap-8">
                        {paginatedInProgressTasks.map((item) => (
                            <TaskCard
                                key={item.task.id}
                                title={item.task.name}
                                assignee={item.employeeName}
                        ...
                        <div className="flex justify-between items-center mt-4">
                            <button
                                ...
                                onClick={() => handlePageChange(setPendingPage, inProgressPage - 1, totalInProgressPages)}
                            >
                                <img src='/arrow.png' width={10} height={10} />
                            </button>
                            <span>{inProgressPage} of {totalInProgressPages}</span>
                            <button
                                className="px-4 py-2 bg-gray-200 rounded"
                                disabled={inProgressPage === totalInProgressPages}
                                onClick={() => handlePageChange(setInProgressPage, inProgressPage + 1, totalInProgressPages)}
                            >
                               <img src='/rightarrow.png' width={10} height={10} />
                            </button>
                        </div>
                    ...
                    <div className="flex-1 flex flex-col gap-8">
                        {paginatedCompletedTasks.map((item) => (
                            <TaskCard
                                key={item.task.id}
                                title={item.task.name}
                                assignee={item.employeeName}
                        ...
                        <div className="flex justify-between items-center mt-4">
                            <button
                                ...
                                onClick={() => handlePageChange(setCompletedPage, completedPage - 1, totalCompletedPages)}
                            >
                                <img src='/arrow.png' width={10} height={10} />
                            </button>
                            <span>{completedPage} of {totalCompletedPages}</span>
                            <button
                                className="px-4 py-2 bg-gray-200 rounded"
